...
test("CLI workflow portal exposes user-story metadata update endpoint for sidebar editing", async () => {
  ...
  assert.match(source, /UpdateUserStoryInfoAsync\([\s\S]*?request\.UsId,[\s\S]*?request\.Title,[\s\S]*?request\.Kind,[\s\S]*?request\.Owner,[\s\S]*?request\.Category,[\s\S]*?request\.Tags,[\s\S]*?request\.ExternalReferences,[\s\S]*?request\.Actor \?\? ResolveCurrentGitOwner\(workspaceRoot\)/);
  assert.match(source, /internal sealed record UpdateUserStoryInfoRequest\(\s*string UsId,\s*string\? Title,\s*string\? Kind,\s*string\? Owner,\s*string\? Category,\s*IReadOnlyList<string>\? Tags,\s*IReadOnlyList<UserStoryExternalReference>\? ExternalReferences,\s*string\? Actor\);/);
});
