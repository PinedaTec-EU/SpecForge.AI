...
export { buildWorkflowAuditHtml } from "./workflow-view/auditView";
import { extractArtifactQuestionBlock, type ArtifactQuestionBlock } from "./workflow-view/artifactQuestions";
import { findConfiguredModelForProfile, formatExecutionLabel } from "./workflow-view/executionLabels";
import { buildRefinementPhaseSections } from "./workflow-view/refinementPhaseView";
import { formatDuration, formatMetricNumber, formatTokensPerSecond } from "./workflow-view/metricFormatters";
...
import {
  ...
  resolveWorkflowGraphLayoutMode,
  type AggregateWorkflowGraphLayoutConfig,
...
type RenderWorkflowGraphNodeConfig = {
  readonly phaseClassName: string;
  readonly visualTone: PhaseVisualTone;
  ...
  readonly targetAttributeName?: "data-phase-id" | "data-us-id";
  readonly targetId?: string;
  readonly extraDataAttributesHtml?: string;
  readonly style: string;
  readonly headerMetaHtml?: string;
  readonly headerActionsHtml?: string;
  readonly visualIconHtml: string;
  readonly title: string;
  readonly secondaryLabel?: string;
  readonly bodyLines?: readonly string[];
};
type AggregateGraphNodeConfig = {
  readonly targetId: string;
  readonly phaseKind?: string;
  readonly command?: "selectPhase" | "openWorkflow";
  readonly title: string;
  readonly phaseTone?: PhaseVisualTone;
  readonly selected?: boolean;
  readonly current?: boolean;
  readonly final?: boolean;
  ...
  readonly statusTag?: {
    readonly tone: "active" | "waiting-user" | "blocked" | "completed" | "pending" | "approval";
...
function findPhaseById(workflow: UserStoryWorkflowDetails, phaseId: string): WorkflowPhaseDetails | null {
  return workflow.phases.find((phase) => phase.phaseId === phaseId) ?? null;
}

function buildAggregateArtifactFlowLabel(inputPath: string | null | undefined, outputPath: string | null | undefined): string {
  const parts: string[] = [];
  if (inputPath) {
...
function buildAggregateGraphLayout(
  childrenCount: number,
  nodeWidth: number,
  config: AggregateWorkflowGraphLayoutConfig
): {
...
function renderWorkflowGraphNode(config: RenderWorkflowGraphNodeConfig): string {
  ...
  const targetDataAttribute = config.targetAttributeName && config.targetId
    ? `${config.targetAttributeName}="${escapeHtmlAttribute(config.targetId)}"`
    : "";
  const secondaryLabel = config.secondaryLabel ? `<div class="phase-slug">${escapeHtml(config.secondaryLabel)}</div>` : "";
  const extraBodyLines = (config.bodyLines ?? [])
    .filter((line) => Boolean(line))
    .map((line) => `<div class="phase-slug">${escapeHtml(line)}</div>`)
    .join("");

  return `
    <div
      class="phase-node ${escapeHtmlAttribute(config.phaseClassName)} phase-tone-${escapeHtmlAttribute(config.visualTone)}${selectedClass}${currentClass}${dependencyBlockedClass}${finalClass}${config.aggregate ? " phase-node--aggregate" : ""}"
      ${config.commandName ? `data-command="${escapeHtmlAttribute(config.commandName)}"` : ""}
      ${targetDataAttribute}
...
function renderPhaseTag(label: string, tone: "active" | "waiting-user" | "blocked" | "completed" | "pending" | "approval" = "approval"): string {
  return `<span class="phase-tag${tone === "approval" ? " approval" : ` phase-tag--${escapeHtmlAttribute(tone)}`}">${escapeHtml(label)}</span>`;
}

function renderGraphStatusBadgeFromTone(tone: PhaseVisualTone, phaseId = "spec"): string {
  return `<span class="phase-role-badge graph-phase-status-icon">${renderGraphPhaseStatusIcon({ phaseId } as WorkflowPhaseDetails, tone, false) || workflowPhaseIcon(phaseId)}</span>`;
}

function renderPhaseRoleBadge(expectsHumanIntervention: boolean): string {
  const phaseRoleIcon = expectsHumanIntervention ? userPhaseIcon() : automationPhaseIcon();
  ...
  return `<span class="phase-role-badge phase-role-badge--${escapeHtmlAttribute(phaseRoleModifier)}" title="${escapeHtmlAttribute(phaseRoleLabel)}" aria-label="${escapeHtmlAttribute(phaseRoleLabel)}">${phaseRoleIcon}</span>`;
}

function renderAggregateNode(config: AggregateGraphNodeConfig): string {
  return renderWorkflowGraphNode({
    ...
    visualIconHtml: config.visualIconHtml ?? workflowPhaseIcon(config.phaseKind ?? "spec"),
    title: config.title,
    secondaryLabel: config.secondaryLabel ?? config.eyebrow,
    bodyLines: config.bodyLines ?? [config.artifact, config.subtitle, config.footer].filter((line): line is string => Boolean(line))
  });
}
...
function buildInspectionPromptModalContent(
  ...
  const sourcePrompts = (prompt.sourcePrompts ?? []).map((item) => `
      <div class="inspection-kv">
        ...
        <span>${escapeHtml(item.isOverride ? "override" : "embedded")}</span>
      </div>
    `).join("");
  const warnings = (prompt.warnings ?? []).map((warning) => `<li>${escapeHtml(warning)}</li>`).join("");
  ...
  return `
    <div class="inspection-stack">
      ...
      <section class="detail-card detail-card--embedded-artifact">
        <h3>System Prompt</h3>
        <pre class="artifact-preview artifact-preview--raw-artifact">${escapeHtml(prompt.systemPrompt)}</pre>
      </section>
      <section class="detail-card detail-card--embedded-artifact">
        <h3>User Prompt</h3>
        <pre class="artifact-preview artifact-preview--raw-artifact">${escapeHtml(prompt.userPrompt)}</pre>
      </section>
    </div>
  `;
}
  ...
  const renderArtifacts = (items: readonly { path: string; phaseId?: string | null; sha256?: string | null }[]) =>
    items.length === 0
      ...
      : `<div class="inspection-grid">${items.map((item) => `
          <div class="inspection-kv">
            <strong>${escapeHtml(item.phaseId ?? fileNameFromPath(item.path))}</strong>
            <span>${escapeHtml(item.path)}</span>
            <span>${escapeHtml(item.sha256 ?? "(no hash)")}</span>
          </div>
        `).join("")}</div>`;
    ...
    <div class="inspection-stack">
      ...
      <section class="detail-card detail-card--embedded-artifact">
        ...
        <div class="inspection-grid">
          <div class="inspection-kv"><strong>Workspace Root</strong><span>${escapeHtml(context.workspaceRoot)}</span></div>
          <div class="inspection-kv"><strong>User Story Path</strong><span>${escapeHtml(context.userStoryPath)}</span></div>
          <div class="inspection-kv"><strong>Workspace Git HEAD</strong><span>${escapeHtml(context.workspaceGitHeadSha ?? "(unavailable)")}</span></div>
          <div class="inspection-kv"><strong>Operation Prompt SHA256</strong><span>${escapeHtml(context.operationPromptSha256 ?? "(none)")}</span></div>
          <div class="inspection-kv"><strong>Current Artifact</strong><span>${escapeHtml(context.currentArtifact?.path ?? "(none)")}</span></div>
        </div>
      </section>
      <section class="detail-card detail-card--embedded-artifact">
...
<body>
    ...
    <div class="workflow-files-dialog panel" role="dialog" aria-modal="true" aria-labelledby="workflow-effective-prompt-title">
      <div class="workflow-files-dialog__head">
        <div>
          ...
          <h2 id="workflow-effective-prompt-title">${escapeHtml(selectedPhase.title)} effective prompt</h2>
  ...
  <div class="workflow-files-overlay" data-effective-context-overlay hidden>
    <div class="workflow-files-dialog panel" role="dialog" aria-modal="true" aria-labelledby="workflow-effective-context-title">
      <div class="workflow-files-dialog__head">
        <div>
          <p class="eyebrow">Execution Inspection</p>
          <h2 id="workflow-effective-context-title">${escapeHtml(selectedPhase.title)} effective context</h2>
  ...
  <script nonce="${scriptNonce}">
    ...
    const aggregateGraphLayoutConfig = ${JSON.stringify(
      state.workflowGraphLayout?.aggregateUserStories?.[workflow.usId]
...
function buildAggregateWorkflowGraph(
  workflow: UserStoryWorkflowDetails,
  selectedPhaseId: string,
  workflowGraphLayout: WorkflowViewState["workflowGraphLayout"],
  graphLayoutEditMode: boolean
): string {
  ...
  const captureNode = renderAggregateNode({
    targetId: "capture",
    phaseKind: "capture",
    title: graphPhaseTitle(capturePhase ?? { phaseId: "capture", title: "Capture" } as WorkflowPhaseDetails),
    secondaryLabel: graphPhaseSecondaryLabel(capturePhase ?? { phaseId: "capture", title: "Capture" } as WorkflowPhaseDetails),
  ...
  const refinementNode = renderAggregateNode({
    ...
    title: graphPhaseTitle(refinementPhase ?? { phaseId: "refinement", title: "Refinement" } as WorkflowPhaseDetails),
    secondaryLabel: graphPhaseSecondaryLabel(refinementPhase ?? { phaseId: "refinement", title: "Refinement" } as WorkflowPhaseDetails),
  ...
  const specNode = renderAggregateNode({
    targetId: "spec",
    phaseKind: "spec",
    title: graphPhaseTitle(specPhase ?? { phaseId: "spec", title: "Spec" } as WorkflowPhaseDetails),
    secondaryLabel: graphPhaseSecondaryLabel(specPhase ?? { phaseId: "spec", title: "Spec" } as WorkflowPhaseDetails),
  ...
  const aggregateEdgeConnections: Record<string, WorkflowGraphEdgeConnection> = {
    "capture->refinement": { from: "R3", to: "L3" },
    "refinement->spec": { from: "R3", to: "L3" },
    "spec->split": { from: "B3", to: "T3" }
  };
  const aggregateEdges: PhaseGraphEdge[] = [
  ...
  return `
    <div class="phase-graph phase-graph--aggregate${graphLayoutEditMode ? " phase-graph--layout-edit" : ""}" data-graph-layout-mode="vertical" aria-label="Aggregate user story graph" style="--aggregate-graph-width: ${aggregateLayout.width}px; --aggregate-graph-height: ${aggregateLayout.height}px; --aggregate-node-width: ${aggregateNodeWidth}px;">
      ...
      <div class="aggregate-graph-caption">
        ...
        <span class="phase-tag phase-tag--waiting-user">${escapeHtml(workflow.status)}</span>
      ...
      ${childNodes || renderWorkflowGraphNode({
        ...
        visualIconHtml: workflowPhaseIcon("capture"),
...
function renderGraphEdges(
  ...
  return [...edges]
    .sort((left: PhaseGraphEdge, right: PhaseGraphEdge) => (classPriority[left.className] ?? 0) - (classPriority[right.className] ?? 0))
    .map((edge: PhaseGraphEdge) => `<path class="${edge.className}" data-edge="${escapeHtmlAttribute(`${edge.fromPhaseId}->${edge.toPhaseId}`)}" d="${graphPath(edge.fromPhaseId, edge.toPhaseId, positions, nodeWidth, graphLayoutMode, edgeConnections[`${edge.fromPhaseId}->${edge.toPhaseId}`])}"></path>`)
    .join("");
}
