export function playIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M8 5.14v13.72c0 .72.78 1.17 1.4.8l10.2-6.86a.94.94 0 0 0 0-1.6L9.4 4.34A.94.94 0 0 0 8 5.14Z"></path>
    </svg>
  `;
}

export function rewindIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M11.14 6.47a1 1 0 0 1 0 1.41L7.01 12l4.13 4.12a1 1 0 1 1-1.42 1.42l-4.83-4.83a1 1 0 0 1 0-1.42l4.83-4.83a1 1 0 0 1 1.42 0Zm8 0a1 1 0 0 1 0 1.41L15.01 12l4.13 4.12a1 1 0 1 1-1.42 1.42l-4.83-4.83a1 1 0 0 1 0-1.42l4.83-4.83a1 1 0 0 1 1.42 0Z"></path>
    </svg>
  `;
}

export function firstPhaseRewindIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M6 5.25A1.25 1.25 0 0 1 7.25 4h.5A1.25 1.25 0 0 1 9 5.25v13.5A1.25 1.25 0 0 1 7.75 20h-.5A1.25 1.25 0 0 1 6 18.75V5.25Zm13.14 1.22a1 1 0 0 1 0 1.41L15.01 12l4.13 4.12a1 1 0 1 1-1.42 1.42l-4.83-4.83a1 1 0 0 1 0-1.42l4.83-4.83a1 1 0 0 1 1.42 0Z"></path>
    </svg>
  `;
}

export function pauseIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M7 5.5A1.5 1.5 0 0 1 8.5 4h1A1.5 1.5 0 0 1 11 5.5v13A1.5 1.5 0 0 1 9.5 20h-1A1.5 1.5 0 0 1 7 18.5v-13Zm6 0A1.5 1.5 0 0 1 14.5 4h1A1.5 1.5 0 0 1 17 5.5v13a1.5 1.5 0 0 1-1.5 1.5h-1A1.5 1.5 0 0 1 13 18.5v-13Z"></path>
    </svg>
  `;
}

export function stopIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M7 7.5A1.5 1.5 0 0 1 8.5 6h7A1.5 1.5 0 0 1 17 7.5v7a1.5 1.5 0 0 1-1.5 1.5h-7A1.5 1.5 0 0 1 7 14.5v-7Z"></path>
    </svg>
  `;
}

export function fileIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M7.5 3A2.5 2.5 0 0 0 5 5.5v13A2.5 2.5 0 0 0 7.5 21h9a2.5 2.5 0 0 0 2.5-2.5V9.2a2.5 2.5 0 0 0-.73-1.77l-3.7-3.7A2.5 2.5 0 0 0 12.8 3H7.5Zm5.3 1.75c.2 0 .39.08.53.22l3.7 3.7c.14.14.22.33.22.53v9.3c0 .41-.34.75-.75.75h-9a.75.75 0 0 1-.75-.75v-13c0-.41.34-.75.75-.75h5.3Zm-3.55 6.5h5.5a.75.75 0 0 1 0 1.5h-5.5a.75.75 0 0 1 0-1.5Zm0 3.5h5.5a.75.75 0 0 1 0 1.5h-5.5a.75.75 0 0 1 0-1.5Z"></path>
    </svg>
  `;
}

export function externalLinkIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M14.25 4a.75.75 0 0 0 0 1.5h3.19l-7.72 7.72a.75.75 0 1 0 1.06 1.06L18.5 6.56v3.19a.75.75 0 0 0 1.5 0V4.75A.75.75 0 0 0 19.25 4h-5Zm-7.5 3A2.75 2.75 0 0 0 4 9.75v7.5A2.75 2.75 0 0 0 6.75 20h7.5A2.75 2.75 0 0 0 17 17.25v-4a.75.75 0 0 0-1.5 0v4c0 .69-.56 1.25-1.25 1.25h-7.5c-.69 0-1.25-.56-1.25-1.25v-7.5c0-.69.56-1.25 1.25-1.25h4a.75.75 0 0 0 0-1.5h-4Z"></path>
    </svg>
  `;
}

export function cameraIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M9.7 4.5a1.75 1.75 0 0 0-1.43.74l-.82 1.16H6.25A3.25 3.25 0 0 0 3 9.65v7.1A3.25 3.25 0 0 0 6.25 20h11.5A3.25 3.25 0 0 0 21 16.75v-7.1a3.25 3.25 0 0 0-3.25-3.25h-1.2l-.82-1.16a1.75 1.75 0 0 0-1.43-.74H9.7Zm0 1.5h4.6c.08 0 .16.04.2.1l1.04 1.46c.28.4.74.64 1.23.64h1.98c.97 0 1.75.78 1.75 1.75v7.1c0 .97-.78 1.75-1.75 1.75H6.25c-.97 0-1.75-.78-1.75-1.75v-7.1c0-.97.78-1.75 1.75-1.75h1.98c.5 0 .95-.24 1.24-.64l1.03-1.45c.05-.07.12-.11.2-.11ZM12 8.75A4.25 4.25 0 1 0 12 17.25 4.25 4.25 0 0 0 12 8.75Zm0 1.5A2.75 2.75 0 1 1 12 15.75 2.75 2.75 0 0 1 12 10.25Z"></path>
    </svg>
  `;
}

export function lockClosedIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M8.5 10V8.25a3.5 3.5 0 1 1 7 0V10h.75A1.75 1.75 0 0 1 18 11.75v7.5A1.75 1.75 0 0 1 16.25 21h-8.5A1.75 1.75 0 0 1 6 19.25v-7.5A1.75 1.75 0 0 1 7.75 10h.75Zm1.5 0h4V8.25a2 2 0 1 0-4 0V10Z"></path>
    </svg>
  `;
}

export function lockOpenIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M15 10V8.25a2 2 0 1 0-4 0 .75.75 0 0 1-1.5 0 3.5 3.5 0 1 1 7 0V10h.75A1.75 1.75 0 0 1 19 11.75v7.5A1.75 1.75 0 0 1 17.25 21h-8.5A1.75 1.75 0 0 1 7 19.25v-7.5A1.75 1.75 0 0 1 8.75 10H15Zm2.5 1.75A.25.25 0 0 0 17.25 11h-8.5a.25.25 0 0 0-.25.25v7.5c0 .14.11.25.25.25h8.5a.25.25 0 0 0 .25-.25v-7.5Z"></path>
    </svg>
  `;
}

export function userPhaseIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <circle cx="12" cy="8" r="5"></circle>
      <path d="M20 21a8 8 0 0 0-16 0"></path>
    </svg>
  `;
}

export function automationPhaseIcon(): string {
  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M12 8V4H8"></path>
      <rect width="16" height="12" x="4" y="8" rx="3"></rect>
      <path d="M2 14h2"></path>
      <path d="M20 14h2"></path>
      <path d="M15 13v2"></path>
      <path d="M9 13v2"></path>
    </svg>
  `;
}

export function workflowPhaseIcon(phaseId: string): string {
  switch (phaseId) {
    case "capture":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M7 3.75A2.75 2.75 0 0 0 4.25 6.5v11A2.75 2.75 0 0 0 7 20.25h10A2.75 2.75 0 0 0 19.75 17.5V9.85a2.75 2.75 0 0 0-.8-1.94l-3.06-3.06A2.75 2.75 0 0 0 13.94 4H7Zm6.5 1.75c.27 0 .52.1.71.29l3 3a1 1 0 0 1 .29.71v8c0 .55-.45 1-1 1H7a1 1 0 0 1-1-1v-11c0-.55.45-1 1-1h6.5Zm-5 4a.75.75 0 0 1 .75-.75h5.5a.75.75 0 0 1 0 1.5h-5.5A.75.75 0 0 1 8.5 9.5Zm0 3.5a.75.75 0 0 1 .75-.75h5.5a.75.75 0 0 1 0 1.5h-5.5a.75.75 0 0 1-.75-.75Z"></path>
        </svg>
      `;
    case "refinement":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M12 3.5c-4.7 0-8.5 3.46-8.5 7.72 0 2.28 1.11 4.34 3.04 5.75.34.25.54.65.54 1.08v1.2c0 .69.56 1.25 1.25 1.25h7.34c.69 0 1.25-.56 1.25-1.25v-1.2c0-.43.2-.83.54-1.08 1.93-1.4 3.04-3.47 3.04-5.75C20.5 6.96 16.7 3.5 12 3.5Zm0 1.5c3.86 0 7 2.8 7 6.22 0 1.8-.89 3.46-2.43 4.58-.71.52-1.11 1.33-1.11 2.25v.95H8.54v-.95c0-.92-.4-1.73-1.11-2.25C5.89 14.68 5 13.02 5 11.22 5 7.8 8.14 5 12 5Zm-.15 3a2.88 2.88 0 0 0-2.85 2.53.75.75 0 0 0 1.49.16c.08-.71.67-1.19 1.36-1.19.77 0 1.4.57 1.4 1.27 0 .46-.18.73-.95 1.28-.84.6-1.45 1.32-1.45 2.45v.25a.75.75 0 0 0 1.5 0v-.18c0-.48.2-.8.82-1.25.93-.67 1.58-1.45 1.58-2.55 0-1.53-1.3-2.77-2.9-2.77Zm.15 9.25a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z"></path>
        </svg>
      `;
    case "spec":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M7 4.25A2.75 2.75 0 0 0 4.25 7v10A2.75 2.75 0 0 0 7 19.75h10A2.75 2.75 0 0 0 19.75 17V7A2.75 2.75 0 0 0 17 4.25H7Zm0 1.5h10c.69 0 1.25.56 1.25 1.25v10c0 .69-.56 1.25-1.25 1.25H7A1.25 1.25 0 0 1 5.75 17V7c0-.69.56-1.25 1.25-1.25Zm1.75 2.5a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5Zm0 3.5a.75.75 0 0 0 0 1.5h4.25a.75.75 0 0 0 0-1.5H8.75Zm0 3.5a.75.75 0 0 0 0 1.5h5.5a.75.75 0 0 0 0-1.5h-5.5Z"></path>
        </svg>
      `;
    case "technical-design":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M5.75 4A1.75 1.75 0 0 0 4 5.75v12.5C4 19.22 4.78 20 5.75 20h12.5c.97 0 1.75-.78 1.75-1.75V5.75C20 4.78 19.22 4 18.25 4H5.75Zm0 1.5h12.5c.14 0 .25.11.25.25v12.5a.25.25 0 0 1-.25.25H5.75a.25.25 0 0 1-.25-.25V5.75c0-.14.11-.25.25-.25Zm2.5 2.25a.75.75 0 0 0-.53 1.28l1.47 1.47-1.47 1.47a.75.75 0 1 0 1.06 1.06l1.47-1.47 1.47 1.47a.75.75 0 1 0 1.06-1.06l-1.47-1.47 1.47-1.47a.75.75 0 1 0-1.06-1.06l-1.47 1.47-1.47-1.47a.75.75 0 0 0-.53-.22Zm7.75.5a.75.75 0 0 0-.75.75v6a.75.75 0 0 0 1.5 0V9a.75.75 0 0 0-.75-.75Z"></path>
        </svg>
      `;
    case "implementation":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M8.03 5.47a.75.75 0 0 1 0 1.06L3.56 11l4.47 4.47a.75.75 0 1 1-1.06 1.06l-5-5a.75.75 0 0 1 0-1.06l5-5a.75.75 0 0 1 1.06 0Zm7.94 0a.75.75 0 0 1 1.06 0l5 5a.75.75 0 0 1 0 1.06l-5 5a.75.75 0 0 1-1.06-1.06L20.44 11l-4.47-4.47a.75.75 0 0 1 0-1.06ZM13.8 4.65a.75.75 0 0 1 .56.9l-3.1 12.8a.75.75 0 1 1-1.46-.35l3.1-12.8a.75.75 0 0 1 .9-.55Z"></path>
        </svg>
      `;
    case "review":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M10.25 4.5a5.75 5.75 0 0 1 4.52 9.3l4.01 4.01a.75.75 0 1 1-1.06 1.06l-4-4a5.75 5.75 0 1 1-3.47-10.37Zm0 1.5a4.25 4.25 0 1 0 0 8.5 4.25 4.25 0 0 0 0-8.5Zm2.31 2.66a.75.75 0 0 1 .03 1.06l-2.25 2.38a.75.75 0 0 1-1.08.01L8.1 10.94a.75.75 0 0 1 1.05-1.07l.62.61 1.72-1.79a.75.75 0 0 1 1.06-.03Z"></path>
        </svg>
      `;
    case "release-approval":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M12 3.5c.2 0 .4.04.58.12l6 2.67c.54.24.9.77.9 1.37v3.77c0 4.17-2.48 7.95-6.31 9.62a2.9 2.9 0 0 1-2.34 0c-3.83-1.67-6.31-5.45-6.31-9.62V7.66c0-.6.35-1.13.9-1.37l6-2.67c.18-.08.38-.12.58-.12Zm0 1.63L6.02 7.79v3.64c0 3.57 2.13 6.82 5.4 8.24.37.17.79.17 1.16 0 3.27-1.42 5.4-4.67 5.4-8.24V7.79L12 5.13Zm2.57 4.6a.75.75 0 0 1 .02 1.06l-3.06 3.18a.75.75 0 0 1-1.08.01l-1.52-1.51a.75.75 0 0 1 1.06-1.06l.98.97 2.53-2.63a.75.75 0 0 1 1.07-.02Z"></path>
        </svg>
      `;
    case "pr-preparation":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M7 4.25A2.75 2.75 0 1 1 7 9.75 2.75 2.75 0 0 1 7 4.25Zm0 1.5a1.25 1.25 0 1 0 0 2.5 1.25 1.25 0 0 0 0-2.5Zm10 8.5a2.75 2.75 0 1 1 0 5.5 2.75 2.75 0 0 1 0-5.5Zm0 1.5a1.25 1.25 0 1 0 0 2.5 1.25 1.25 0 0 0 0-2.5ZM7.75 9a.75.75 0 0 1 .75.75v1.58c0 1.46 1.19 2.65 2.65 2.65h1.93a.75.75 0 0 1 0 1.5h-1.93A4.15 4.15 0 0 1 7 11.33V9.75A.75.75 0 0 1 7.75 9Zm8.5-4.75A2.75 2.75 0 1 1 16.25 9.75 2.75 2.75 0 0 1 16.25 4.25Zm0 1.5a1.25 1.25 0 1 0 0 2.5 1.25 1.25 0 0 0 0-2.5Zm-1.08 3.8a.75.75 0 0 1 1.06 0l1.47 1.47 1.47-1.47a.75.75 0 0 1 1.06 1.06l-2 2a.75.75 0 0 1-1.06 0l-2-2a.75.75 0 0 1 0-1.06Z"></path>
        </svg>
      `;
    case "completed":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M12 3.75a1.75 1.75 0 0 1 1.58 1l1.03 2.16 2.36.35a1.75 1.75 0 0 1 .97 2.98l-1.72 1.68.4 2.38a1.75 1.75 0 0 1-2.54 1.84L12 15l-2.08 1.13a1.75 1.75 0 0 1-2.54-1.84l.4-2.38-1.72-1.68a1.75 1.75 0 0 1 .97-2.98l2.36-.35 1.03-2.16a1.75 1.75 0 0 1 1.58-1Zm0 1.5a.25.25 0 0 0-.23.14l-1.2 2.53a.75.75 0 0 1-.56.42l-2.74.4a.25.25 0 0 0-.14.42l1.98 1.94c.18.18.26.43.22.68l-.47 2.73a.25.25 0 0 0 .36.26l2.45-1.33a.75.75 0 0 1 .72 0l2.45 1.33a.25.25 0 0 0 .36-.26l-.47-2.73a.75.75 0 0 1 .22-.68l1.98-1.94a.25.25 0 0 0-.14-.42l-2.74-.4a.75.75 0 0 1-.56-.42l-1.2-2.53a.25.25 0 0 0-.23-.14Z"></path>
        </svg>
      `;
    default:
      return automationPhaseIcon();
  }
}

export function graphLayoutModeIcon(layoutMode: "horizontal" | "vertical"): string {
  if (layoutMode === "horizontal") {
    return `
      <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
        <path d="M4.75 6A1.75 1.75 0 0 0 3 7.75v3.5C3 12.22 3.78 13 4.75 13h3.5C9.22 13 10 12.22 10 11.25v-.5h4v.5c0 .97.78 1.75 1.75 1.75h3.5C20.22 13 21 12.22 21 11.25v-3.5C21 6.78 20.22 6 19.25 6h-3.5C14.78 6 14 6.78 14 7.75v.5h-4v-.5C10 6.78 9.22 6 8.25 6h-3.5ZM4.5 7.75c0-.14.11-.25.25-.25h3.5c.14 0 .25.11.25.25v3.5a.25.25 0 0 1-.25.25h-3.5a.25.25 0 0 1-.25-.25v-3.5Zm11 0c0-.14.11-.25.25-.25h3.5c.14 0 .25.11.25.25v3.5a.25.25 0 0 1-.25.25h-3.5a.25.25 0 0 1-.25-.25v-3.5ZM9.75 16A1.75 1.75 0 0 0 8 17.75v1.5C8 20.22 8.78 21 9.75 21h4.5c.97 0 1.75-.78 1.75-1.75v-1.5c0-.97-.78-1.75-1.75-1.75h-4.5Zm-.25 1.75c0-.14.11-.25.25-.25h4.5c.14 0 .25.11.25.25v1.5a.25.25 0 0 1-.25.25h-4.5a.25.25 0 0 1-.25-.25v-1.5Z"></path>
      </svg>
    `;
  }

  return `
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M6.75 3A1.75 1.75 0 0 0 5 4.75v3.5C5 9.22 5.78 10 6.75 10h.5v4h-.5C5.78 14 5 14.78 5 15.75v3.5C5 20.22 5.78 21 6.75 21h3.5C11.22 21 12 20.22 12 19.25v-3.5c0-.97-.78-1.75-1.75-1.75h-.5v-4h4v4h-.5c-.97 0-1.75.78-1.75 1.75v3.5c0 .97.78 1.75 1.75 1.75h3.5c.97 0 1.75-.78 1.75-1.75v-3.5c0-.97-.78-1.75-1.75-1.75h-.5v-4h.5C18.22 10 19 9.22 19 8.25v-3.5C19 3.78 18.22 3 17.25 3h-3.5C12.78 3 12 3.78 12 4.75v.5H9.75v-.5C9.75 3.78 8.97 3 8 3h-1.25Zm-.25 1.75c0-.14.11-.25.25-.25h3.5c.14 0 .25.11.25.25v3.5a.25.25 0 0 1-.25.25h-3.5a.25.25 0 0 1-.25-.25v-3.5Zm7 0c0-.14.11-.25.25-.25h3.5c.14 0 .25.11.25.25v3.5a.25.25 0 0 1-.25.25h-3.5a.25.25 0 0 1-.25-.25v-3.5Zm-7 11c0-.14.11-.25.25-.25h3.5c.14 0 .25.11.25.25v3.5a.25.25 0 0 1-.25.25h-3.5a.25.25 0 0 1-.25-.25v-3.5Zm7 0c0-.14.11-.25.25-.25h3.5c.14 0 .25.11.25.25v3.5a.25.25 0 0 1-.25.25h-3.5a.25.25 0 0 1-.25-.25v-3.5Z"></path>
    </svg>
  `;
}
