...
exports.escapeHtml = exports.buildWorkflowAuditHtml = void 0;
...
var auditView_1 = require("./workflow-view/auditView");
Object.defineProperty(exports, "buildWorkflowAuditHtml", { enumerable: true, get: function () { return auditView_1.buildWorkflowAuditHtml; } });
const artifactQuestions_1 = require("./workflow-view/artifactQuestions");
const executionLabels_1 = require("./workflow-view/executionLabels");
const refinementPhaseView_1 = require("./workflow-view/refinementPhaseView");
const metricFormatters_1 = require("./workflow-view/metricFormatters");
...
function buildRuntimeVersionMarkup(runtimeVersion) {
    ...
    return displayVersion ? `<span class="runtime-version">v.${(0, htmlEscape_1.escapeHtml)(displayVersion)}</span>` : "";
}
...
function aggregateWorkflowUsage(events, state) {
    ...
    for (const event of events) {
        ...
        const modelLabel = (0, executionLabels_1.formatExecutionLabel)(event.execution, {
...
function renderHarnessProfileSection(harnessProfile, governance) {
    if (!harnessProfile) {
        return "";
    }
...
function buildExecutionOverlay(workflow, state, playbackState) {
    ...
    const overlayConfiguredModel = (0, executionLabels_1.findConfiguredModelForProfile)(state, overlayPhaseProfileLabel);
    const overlayPhaseConfiguredLabel = (0, executionLabels_1.formatExecutionLabel)(overlayConfiguredModel ? { model: overlayConfiguredModel, profileName: overlayPhaseProfileLabel } : null, { configuredModel: overlayConfiguredModel });
...
function buildAssignedPhaseExecutionLabel(phase, state) {
    ...
    const configuredModel = (0, executionLabels_1.findConfiguredModelForProfile)(state, profileName);
    if (!profileName && !configuredModel) {
        return null;
    }
    return (0, executionLabels_1.formatExecutionLabel)(configuredModel ? { model: configuredModel, profileName } : { model: "", profileName }, { configuredModel }) ?? profileName ?? configuredModel;
}
...
function resolveDependencyBlockState(workflow) {
...
function buildDependencyBlockedHeroMarkup(blockState) {
    if (!blockState) {
        return "";
    }
    const dependencyLinks = blockState.dependencies.map((dependency) => {
      ...
      <button
        ...
        data-us-id="${(0, htmlEscape_1.escapeHtmlAttr)(dependency.usId)}"
        title="Open ${(0, htmlEscape_1.escapeHtmlAttr)(dependency.usId)}">
        <span class="dependency-block__link-id">${(0, htmlEscape_1.escapeHtml)(dependency.usId)}</span>
        <span class="dependency-block__link-title">${(0, htmlEscape_1.escapeHtml)(title || "Open dependency")}</span>
        ${meta ? `<span class="dependency-block__link-meta">${(0, htmlEscape_1.escapeHtml)(meta)}</span>` : ""}
        ${missingReason ? `<span class="dependency-block__link-meta">${(0, htmlEscape_1.escapeHtml)(missingReason)}</span>` : ""}
      </button>
    `;
    }).join("");
    ...
    <section class="dependency-block" aria-label="Blocked user story dependencies">
      <div class="dependency-block__icon" aria-hidden="true">${(0, icons_1.lockClosedIcon)()}</div>
      <div class="dependency-block__content">
        <div class="dependency-block__summary">
          ...
          <strong>${(0, htmlEscape_1.escapeHtml)(countLabel)}</strong>
        </div>
...
function buildInspectionPromptModalContent(receiptPath, prompt, emptyMessage) {
    if (!prompt) {
        return `<p class="muted">${(0, htmlEscape_1.escapeHtml)(emptyMessage)}</p>`;
    }
    ...
    <div class="inspection-stack">
      ${receiptPath ? `<p class="muted">Receipt: <code>${(0, htmlEscape_1.escapeHtml)(receiptPath)}</code></p>` : ""}
...
function buildEffectivePromptModalContent(selectedPhase) {
    ...
    return buildInspectionPromptModalContent(inspection?.receiptPath, inspection?.effectivePrompt, "No effective prompt is available for this phase.");
}
function buildInspectionContextModalContent(receiptPath, context, emptyMessage) {
    if (!context) {
        return `<p class="muted">${(0, htmlEscape_1.escapeHtml)(emptyMessage)}</p>`;
    }
...
function buildEffectiveContextModalContent(selectedPhase) {
    const inspection = selectedPhase.latestExecutionInspection;
    return buildInspectionContextModalContent(inspection?.receiptPath, inspection?.effectiveContext, "No effective context is available for this phase.");
}
function getAutoRefinementAnswerInspection(workflow, selectedPhase) {
...
function buildWorkflowHtml(workflow, state, playbackState, typographyCssVars = "", cspSource = "") {
    ...
    const graphStageDesktopHorizontalLegendPosition = (0, graphLayout_1.buildGraphLegendPosition)(state.workflowGraphLayout?.legend?.horizontal?.x ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.horizontal.x, state.workflowGraphLayout?.legend?.horizontal?.y ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.horizontal.y, false);
    const graphStageDesktopVerticalLegendPosition = (0, graphLayout_1.buildGraphLegendPosition)(state.workflowGraphLayout?.legend?.vertical?.x ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.vertical.x, state.workflowGraphLayout?.legend?.vertical?.y ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.vertical.y, false);
    const graphStageMobileHorizontalLegendPosition = (0, graphLayout_1.buildGraphLegendPosition)(state.workflowGraphLayout?.legend?.horizontal?.x ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.horizontal.x, state.workflowGraphLayout?.legend?.horizontal?.y ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.horizontal.y, true);
    const graphStageMobileVerticalLegendPosition = (0, graphLayout_1.buildGraphLegendPosition)(state.workflowGraphLayout?.legend?.vertical?.x ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.vertical.x, state.workflowGraphLayout?.legend?.vertical?.y ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.vertical.y, true);
    ...
    <div class="phase-duration-pill" role="status" aria-label="Phase duration">
      ...
      <div class="phase-duration-pill__body">
        ...
        <span class="phase-duration-pill__value">${(0, htmlEscape_1.escapeHtml)(selectedPhaseDurationAggregate > 0 ? (0, metricFormatters_1.formatDuration)(selectedPhaseDurationAggregate) : "n/a")}</span>
      </div>
    </div>
  `;
    ...
    const tokenSummaryRows = [
        renderTokenSummaryRow("Input / Output", hasTokenTelemetry
            ? `${(0, metricFormatters_1.formatMetricNumber)(selectedPhaseUsageAggregate.inputTokens)} / ${(0, metricFormatters_1.formatMetricNumber)(selectedPhaseUsageAggregate.outputTokens)}`
            : "n/a"),
        renderTokenSummaryRow("Total", hasTokenTelemetry
            ? (0, metricFormatters_1.formatMetricNumber)(selectedPhaseUsageAggregate.totalTokens)
            : "n/a"),
    ...
    if (hasTokenTelemetry && selectedPhaseDurationAggregate > 0) {
        tokenSummaryRows.push(renderTokenSummaryRow("Response Speed", (0, metricFormatters_1.formatTokensPerSecond)(selectedPhaseUsageAggregate.outputTokens, selectedPhaseDurationAggregate)));
    }
    ...
    const lifecycleSummary = selectedPhaseRuntimeMetrics
        ? renderTokenSummaryCard("Lifecycle", [
            ...
            renderTokenSummaryRow("Lead Time", selectedPhaseRuntimeMetrics.leadTimeMs && selectedPhaseRuntimeMetrics.leadTimeMs > 0
                ? (0, metricFormatters_1.formatDuration)(selectedPhaseRuntimeMetrics.leadTimeMs)
                : "n/a"),
            renderTokenSummaryRow("Waiting User", selectedPhaseRuntimeMetrics.waitingUserDurationMs > 0
                ? (0, metricFormatters_1.formatDuration)(selectedPhaseRuntimeMetrics.waitingUserDurationMs)
                : "0s"),
            renderTokenSummaryRow("Blocked", selectedPhaseRuntimeMetrics.blockedDurationMs > 0
                ? (0, metricFormatters_1.formatDuration)(selectedPhaseRuntimeMetrics.blockedDurationMs)
                : "0s")
        ], {
            modifierClass: "token-summary--wide"
        })
    ...
    <section class="detail-card detail-card--workflow-dashboard">
      ...
      <div class="token-summary-grid token-summary-grid--workflow">
        ...
        renderTokenSummaryRow("Input / Output", `${(0, metricFormatters_1.formatMetricNumber)(workflowUsage.overall.inputTokens)} / ${(0, metricFormatters_1.formatMetricNumber)(workflowUsage.overall.outputTokens)}`),
        renderTokenSummaryRow("Total Tokens", (0, metricFormatters_1.formatMetricNumber)(workflowUsage.overall.totalTokens)),
        renderTokenSummaryRow("Recorded Runs", String(workflowUsage.overall.events)),
        renderTokenSummaryRow("Duration", workflowUsage.overall.durationMs > 0 ? (0, metricFormatters_1.formatDuration)(workflowUsage.overall.durationMs) : "n/a"),
        renderTokenSummaryRow("Response Speed", workflowUsage.overall.durationMs > 0 ? (0, metricFormatters_1.formatTokensPerSecond)(workflowUsage.overall.outputTokens, workflowUsage.overall.durationMs) : "n/a")
    ])}
        ...
        ? renderTokenSummaryCard("Lifecycle", [
            ...
            renderTokenSummaryRow("Lead Time", workflowRuntimeMetrics.leadTimeMs && workflowRuntimeMetrics.leadTimeMs > 0
                ? (0, metricFormatters_1.formatDuration)(workflowRuntimeMetrics.leadTimeMs)
                : "n/a"),
            renderTokenSummaryRow("Waiting User", workflowRuntimeMetrics.waitingUserDurationMs > 0
                ? (0, metricFormatters_1.formatDuration)(workflowRuntimeMetrics.waitingUserDurationMs)
                : "0s"),
            renderTokenSummaryRow("Blocked", workflowRuntimeMetrics.blockedDurationMs > 0
                ? (0, metricFormatters_1.formatDuration)(workflowRuntimeMetrics.blockedDurationMs)
                : "0s")
        ])
        : ""}
      </div>
  ...
  `;
    const modelUsageTable = showCompletedOnlyUsagePanels
        ? renderUsageDashboardTable("Usage by Model", ["Model", "Runs", "Input", "Output", "Total", "Duration", "Speed"], workflowUsage.byModel.length > 0
            ? workflowUsage.byModel.map(({ label, aggregate }) => ([
                ...
                (0, metricFormatters_1.formatMetricNumber)(aggregate.inputTokens),
                (0, metricFormatters_1.formatMetricNumber)(aggregate.outputTokens),
                (0, metricFormatters_1.formatMetricNumber)(aggregate.totalTokens),
                aggregate.durationMs > 0 ? (0, metricFormatters_1.formatDuration)(aggregate.durationMs) : "n/a",
                aggregate.durationMs > 0 ? (0, metricFormatters_1.formatTokensPerSecond)(aggregate.outputTokens, aggregate.durationMs) : "n/a"
            ]))
    ...
    const iterationRail = phaseIterations.length > 0
      ...
      <section class="detail-card detail-card--phase-iterations" data-iteration-rail-section data-phase-id="${(0, htmlEscape_1.escapeHtmlAttr)(selectedPhase.phaseId)}">
        ...
        }) ? ` · ${(0, htmlEscape_1.escapeHtml)((0, executionLabels_1.formatExecutionLabel)(iteration.execution, {
            actor: iteration.actor,
            configuredModel: (0, executionLabels_1.findConfiguredModelForProfile)(state, iteration.execution?.profileName)
        }) ?? "")}` : ""}
                  ${iteration.usage ? ` · ${(0, htmlEscape_1.escapeHtml)(`${(0, metricFormatters_1.formatMetricNumber)(iteration.usage.inputTokens)}/${(0, metricFormatters_1.formatMetricNumber)(iteration.usage.outputTokens)} tok`)}` : ""}
                  ${iteration.durationMs !== null ? ` · ${(0, htmlEscape_1.escapeHtml)((0, metricFormatters_1.formatDuration)(iteration.durationMs))}` : ""}
                </span>
      ...
      <section class="detail-card detail-card--iteration-detail">
        ...
        <div class="iteration-detail__meta">
          ...
          ${(0, executionLabels_1.formatExecutionLabel)(selectedIteration.execution, {
            ...
            configuredModel: (0, executionLabels_1.findConfiguredModelForProfile)(state, selectedIteration.execution?.profileName)
        }) ? `<span class="badge">model ${(0, htmlEscape_1.escapeHtml)((0, executionLabels_1.formatExecutionLabel)(selectedIteration.execution, {
            actor: selectedIteration.actor,
            configuredModel: (0, executionLabels_1.findConfiguredModelForProfile)(state, selectedIteration.execution?.profileName)
        }) ?? "")}</span>` : ""}
          ${selectedIteration.usage ? `<span class="badge">in/out ${(0, htmlEscape_1.escapeHtml)(`${(0, metricFormatters_1.formatMetricNumber)(selectedIteration.usage.inputTokens)}/${(0, metricFormatters_1.formatMetricNumber)(selectedIteration.usage.outputTokens)}`)}</span>` : ""}
          ${selectedIteration.usage ? `<span class="badge">total ${(0, htmlEscape_1.escapeHtml)((0, metricFormatters_1.formatMetricNumber)(selectedIteration.usage.totalTokens))}</span>` : ""}
...
<body>
  ...
  <div class="workflow-files-overlay" data-auto-refinement-prompt-overlay hidden>
    <div class="workflow-files-dialog panel" role="dialog" aria-modal="true" aria-labelledby="workflow-auto-refinement-prompt-title">
      ...
      <div class="workflow-files-shell">
        ...
        return buildInspectionPromptModalContent(inspection?.receiptPath, inspection?.effectivePrompt, "No auto-answer effective prompt is available for this user story.");
    })()}
      </div>
    </div>
  </div>
  <div class="workflow-files-overlay" data-auto-refinement-context-overlay hidden>
    <div class="workflow-files-dialog panel" role="dialog" aria-modal="true" aria-labelledby="workflow-auto-refinement-context-title">
      ...
      <div class="workflow-files-shell">
        ${(() => {
        const inspection = getAutoRefinementAnswerInspection(workflow, selectedPhase);
        return buildInspectionContextModalContent(inspection?.receiptPath, inspection?.effectiveContext, "No auto-answer effective context is available for this user story.");
    })()}
      </div>
    </div>
  </div>
          ...
          <div class="graph-panel__head">
            ...
            <div class="graph-stage-actions">
              ...
              <button
                ...
                ${(0, icons_1.graphLayoutModeIcon)("vertical")}
              </button>
              <button
                class="graph-stage-action-button"
                type="button"
  ...
  <script nonce="${scriptNonce}">
    ...
    const graphLayoutModeIcons = ${JSON.stringify({
        horizontal: (0, icons_1.graphLayoutModeIcon)("horizontal"),
        vertical: (0, icons_1.graphLayoutModeIcon)("vertical")
    })};
...
function buildPhaseGraph(workflow, state, selectedPhaseId, playbackState, effectiveExecutionPhaseId) {
    ...
    const visiblePhases = graphPhases.filter((phase) => shouldShowPhase(phase.phaseId, refinementVisible, currentPhase.phaseId, executionPhaseId));
    ...
    const mobileVerticalLegendPosition = (0, graphLayout_1.buildGraphLegendPosition)(state.workflowGraphLayout?.legend?.vertical?.x ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.vertical.x, state.workflowGraphLayout?.legend?.vertical?.y ?? workflowGraphLayout_1.defaultWorkflowGraphLegendPositions.vertical.y, true);
...
function ensureWorkflowGraphPhases(workflow) {
