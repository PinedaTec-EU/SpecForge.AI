    ...
    public static JsonObject BuildToolsList()
    {
        return new JsonObject
        {
            ["tools"] = new JsonArray
            {
                ...
                Tool("specforge_action", "Compact SpecForge mutation facade for Codex clients. Use this instead of editing .specs files directly.",
                    Schema(
                        ...
                        Props(
                            ...
                            ("params",        Prop("object", "Action-specific parameters. update_user_story_info accepts title, kind, owner, category, tags, and actor."))))),
                ...
                Tool("approve_decomposition", "Approve a pending spec decomposition proposal and create child user stories.",
                    Schema(required: ["workspaceRoot", "usId"], Props(("workspaceRoot", Prop("string", "Absolute path to the workspace root.")), ("usId", Prop("string", "User story identifier.")), ("actor", Prop("string", "Actor performing the approval. Defaults to 'user'."))))),
                Tool("reject_decomposition", "Reject a pending spec decomposition proposal so the parent spec can proceed through normal approval.",
                    Schema(required: ["workspaceRoot", "usId"], Props(("workspaceRoot", Prop("string", "Absolute path to the workspace root.")), ("usId", Prop("string", "User story identifier.")), ("actor", Prop("string", "Actor performing the rejection. Defaults to 'user'."))))),
