...
public sealed class SpecForgeApplicationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var (phaseId, index) in phases.Select((phaseId, index) => (phaseId, index)))
        {
            ...
            var specApprovalPolicy = phaseId == PhaseId.Spec
                ? await BuildSpecApprovalPolicyDetailsAsync(workflowRun, paths, cancellationToken)
                : null;
    ...
    private static async Task<SpecPhaseApprovalPolicyDetails> BuildSpecApprovalPolicyDetailsAsync(
        Workflow.WorkflowRun workflowRun,
        UserStoryFilePaths paths,
        CancellationToken cancellationToken)
    {
        var isCurrentSpecPhase = workflowRun.CurrentPhase == Workflow.PhaseId.Spec && workflowRun.Status != UserStoryStatus.Completed;
        var isApproved = workflowRun.IsPhaseApproved(Workflow.PhaseId.Spec);
        var decomposition = await ReadDecompositionDetailsAsync(paths, cancellationToken);
        var decompositionPendingApproval = decomposition?.State == UserStoryDecomposition.StatePendingApproval;
        var specPath = paths.GetLatestExistingPhaseArtifactPath(Workflow.PhaseId.Spec);
